...
import PlatformRecoveryReadinessPage from "@/components/platform-admin/PlatformRecoveryReadinessPage";
import { getHqConfig, HQ_COOKIE_NAME, verifyHqSession } from "@/lib/hq-auth";
import { createServerSupabaseClient } from "@/lib/supabase/server";

export const dynamic = "force-dynamic";

export default async function PlatformRecoveryReadinessRoute() {
  const supabase = await createServerSupabaseClient();
  if (!supabase) redirect("/login");

  const { data: claimsData } = await supabase.auth.getClaims();
  const userId = claimsData?.claims?.sub;
  if (!userId) redirect("/login?next=/admin/recovery");
  ...
  return <PlatformRecoveryReadinessPage />;
}
