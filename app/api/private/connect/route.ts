...
async function connectContext() {
  ...
  return { ok: true as const, supabase: auth.supabase, userId: auth.userId, profileId: profile.id, organizationId: profile.current_organization_id, role: membership.role };
}
...
export async function GET() {
  ...
  return Response.json({
    ...
    messages: (messages.data ?? []).map((message) => ({
      ...message,
      author_name: message.author_user_id === context.userId ? "Du" : message.author_worker_id ? workerNames.get(message.author_worker_id) ?? null : null,
    })),
...
export async function POST(request: Request) {
  ...
  if (action === "send_message") {
    ...
    const { data: worker } = await context.supabase.from("workers").select("id").eq("organization_id", context.organizationId).eq("profile_id", context.profileId).eq("active", true).maybeSingle();
