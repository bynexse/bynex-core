...
function attachmentFrom(value: unknown): Attachment | null {
  ...
  const id = string(item.id, 240) || string(item.attachment_id, 240);
  const filename = safeFilename(string(item.filename, 240));
  const contentType = string(
    item.content_type ?? item.contentType ?? item.mime_type,
    160,
  ).toLowerCase();
  const sizeValue = Number(item.size ?? item.size_bytes);
  ...
  return {
    ...
    contentId: string(item.content_id ?? item.contentId, 300) || null,
...
async function resendJson(path: string, apiKey: string) {
  ...
  if (!response.ok) {
    throw new Error(
      `Resend kunde inte hämta det mottagna mejlet (HTTP ${response.status})`,
    );
  }
...
async function attachmentBytes(
  ...
  if (attachment.downloadUrl) {
    const response = await fetch(trustedResendDownload(attachment.downloadUrl), {
      headers: { Authorization: `Bearer ${apiKey}` },
      signal: AbortSignal.timeout(30_000),
    });
    if (!response.ok) {
      throw new Error(`Bilagan kunde inte hämtas (HTTP ${response.status})`);
    }
    const bytes = new Uint8Array(await response.arrayBuffer());
    if (bytes.byteLength < 1 || bytes.byteLength > MAX_ATTACHMENT_SIZE) {
      throw new Error("Bilagans filstorlek är inte tillåten");
    }
    return bytes;
  }

  const metadata = await resendJson(
    `/emails/receiving/${encodeURIComponent(providerEmailId)}/attachments/${encodeURIComponent(attachment.id)}`,
    apiKey,
  );
  const url = string(
    metadata.download_url ?? metadata.downloadUrl ?? metadata.url,
    2000,
  );
  if (!url) throw new Error("Bilagan saknar en säker nedladdningsadress");
  const response = await fetch(trustedResendDownload(url), {
    headers: { Authorization: `Bearer ${apiKey}` },
    signal: AbortSignal.timeout(30_000),
  });
  if (!response.ok) {
...
async function storeAttachment(input: {
  ...
  if (duplicate) {
    return {
      ...
      supplierInvoiceId: duplicate.supplier_invoice_id,
    };
  }
  ...
  const storagePath = `${input.inbox.organization_id}/${bynexDocumentId}/${input.attachment.filename}`;
...
export async function POST(request: Request) {
  ...
  try {
    event = object(JSON.parse(rawBody) as unknown);
  } catch {
    return Response.json({ error: "Webhookens JSON är ogiltig" }, { status: 400 });
  }
  ...
  const { data: replay } = await client
    ...
    .maybeSingle();
  if (replay) {
    return Response.json({ accepted: true, replay: true, status: replay.status });
  }
  ...
  const receivedAt =
    string(receivedEmail.created_at || eventData.created_at, 80) ||
  ...
  const attachments = (
    Array.isArray(receivedEmail.attachments)
  ...
  const accepted = attachments.filter(acceptedAttachment);
  ...
  const { error: messageError } = await client
    ...
    .insert({
      ...
      accepted_attachment_count: accepted.length,
      status: accepted.length ? "processing" : "ignored",
      raw_metadata: {
        event_type: event.type,
        rejected_attachment_count: attachments.length - accepted.length,
      },
    });
  if (messageError) {
    if (messageError.code === "23505") {
      return Response.json({ accepted: true, replay: true });
    }
    return Response.json({ error: "Mottagningsloggen kunde inte skapas" }, { status: 503 });
  }
  ...
  if (accepted.length === 0) {
    await client
      ...
      .eq("organization_id", inbox.organization_id)
      .eq("id", inbox.id);
    return Response.json({ accepted: true, stored: 0, ignored: attachments.length });
  }
  ...
  for (const attachment of accepted) {
    try {
      const bytes = await attachmentBytes(attachment, providerEmailId, apiKey);
      ...
      failures.push(cause instanceof Error ? cause.message : "Bilagan kunde inte behandlas");
    }
  }
  ...
  await markMessage(client, inbox.organization_id, inboundMessageId, {
    status,
    error_code: failures.length ? "attachment_processing_failed" : null,
