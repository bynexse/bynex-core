...
const questions: Array<[string, string]> = [
  ...
  ["Kan en enskild firma senare bli aktiebolag?", "Ja. Det planerade ombildningsflödet skapar det nya juridiska företaget från rätt startdatum och återanvänder tillåtna register, samtidigt som den enskilda firmans gamla fakturor och bokföring ligger kvar oförändrade."],
...
const plans = [
  ...
  },
  {
    ...
    price: 399,
    firstYear: "199,50",
    audience: "Helheten för den nystartade enskilda företagaren.",
    users: "1 ägare + redovisningsåtkomst",
    badge: "Nystartad",
    features: ["Bynex Tid, offert och fakturering", "Kvitton och leverantörsfakturor", "Egna uttag och uppskattat disponibelt saldo", "Bokföring, moms och förenklat årsbokslut när modulen är klar"],
  },
  {
    name: "Bynex Bygg",
    price: 899,
...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f4f2] text-zinc-950">
      ...
      <section id="funktioner" className="scroll-mt-20 border-y border-zinc-200 bg-white px-5 py-20 md:px-10">
      ...
      <section id="priser" className="scroll-mt-20 border-y border-zinc-200 bg-white px-5 py-20 md:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[1fr_0.7fr] lg:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-emerald-700">Tydliga lanseringspriser</p><h2 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">Börja komplett. Välj om efter första året.</h2><p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">Nystartade företag får 50 % rabatt i 12 månader på vald paketnivå. Alla andra nya Bynex-kunder får 30 % rabatt de första tre månaderna. Inför nästa period kan företaget fortsätta, byta nivå eller välja enskilda moduler.</p></div><div className="space-y-3"><div className="rounded-3xl bg-emerald-50 p-6 text-emerald-950"><div className="flex items-center gap-3"><Tag className="h-6 w-6" /><p className="font-semibold">Nystartad 50 % första året</p></div><p className="mt-3 text-sm leading-6">Gäller företag som startats inom de senaste 12 månaderna.</p></div><div className="rounded-3xl bg-zinc-950 p-6 text-white"><p className="font-semibold">Ny kund: 30 % i tre månader</p><p className="mt-3 text-sm leading-6 text-zinc-300">För företag som byter till Bynex. Det bästa giltiga erbjudandet används; rabatterna kombineras inte.</p></div></div></div>

          <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{plans.map((plan) => <article key={plan.name} className={`flex flex-col rounded-[2rem] border p-6 ${plan.featured ? "border-emerald-500 bg-emerald-50 shadow-lg" : "border-zinc-200"}`}><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-semibold">{plan.name}</h3>{plan.badge && <span className={`rounded-full px-3 py-1 text-[11px] font-bold ${plan.featured ? "bg-emerald-700 text-white" : "bg-zinc-100 text-zinc-700"}`}>{plan.badge}</span>}</div><p className="mt-3 min-h-16 text-sm leading-6 text-zinc-600">{plan.audience}</p><div className="mt-6"><p className="text-4xl font-semibold">{plan.price.toLocaleString("sv-SE")} kr</p><p className="mt-1 text-xs text-zinc-500">per företag/mån exkl. moms · 12 månader</p></div><div className="mt-5 rounded-2xl bg-white/80 p-4"><p className="text-xs font-bold uppercase tracking-wider text-emerald-800">Nystartad första året</p><p className="mt-2 text-2xl font-semibold">{plan.firstYear} kr/mån</p></div><p className="mt-5 flex items-center gap-2 text-sm font-semibold"><UsersRound className="h-4 w-4" /> {plan.users}</p><ul className="mt-5 flex-1 space-y-3">{plan.features.map((feature) => <li key={feature} className="flex items-start gap-2 text-sm leading-6 text-zinc-600"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-emerald-700" />{feature}</li>)}</ul><Link href="/signup" className={`mt-7 inline-flex items-center justify-center rounded-2xl px-5 py-3 text-sm font-semibold ${plan.featured ? "bg-emerald-700 text-white" : "bg-zinc-950 text-white"}`}>Prova utan betalning</Link></article>)}</div>
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-[1fr_0.7fr]">
            ...
            <div className="rounded-[2rem] bg-zinc-950 p-7 text-white"><UsersRound className="h-7 w-7 text-emerald-400" /><h3 className="mt-6 text-2xl font-semibold">Väx utan att byta system</h3><p className="mt-4 text-sm leading-6 text-zinc-300">Extra användare kostar från 99 kr/mån beroende på paket. Företag med minst 25 användare får ett anpassat volympris. En enskild firma kan senare gå över till aktiebolag och behålla sin historik i Bynex.</p><p className="mt-4 text-xs leading-5 text-zinc-400">Rabatter kombineras inte. Nystartad med 24 månaders bindning får 50 % under månad 1–12 och därefter 10 % under återstående bindningstid.</p></div>
          </div>

          <div className="mt-14 grid gap-6 rounded-[2rem] bg-emerald-950 p-7 text-white lg:grid-cols-[0.65fr_1.35fr] lg:items-center sm:p-9">
            <div><BookOpenCheck className="h-8 w-8 text-emerald-300" /><p className="mt-6 text-sm font-bold uppercase tracking-[0.18em] text-emerald-300">Bynex Bokföring · planerat lanseringspris 349 kr/mån</p><h3 className="mt-3 text-3xl font-semibold">Målsättning: ordinarie bokslut utan separat avgift.</h3></div>
            <div><p className="text-lg leading-8 text-emerald-50">I den planerade produktionsversionen ska Bynex Smart kunna förbereda bokslutet, kontrollera underlaget och visa vad som behöver granskas före signering. Målet är stöd för förenklat årsbokslut och NE-underlag för enskild firma samt K2-underlag för mindre aktiebolag.</p><p className="mt-4 text-sm leading-6 text-emerald-200">Funktionerna kan inte köpas eller användas för inlämning innan svenska regelkontroller och officiella kopplingar är verifierade. Revision, K3, koncernredovisning och komplicerade rättelser kan kräva specialist.</p></div>
          </div>

          <div className="mt-6 grid gap-6 rounded-[2rem] border border-emerald-200 bg-emerald-50 p-7 lg:grid-cols-[0.7fr_1.3fr] lg:items-center sm:p-9">
            <div><HousePlug className="h-8 w-8 text-emerald-800" /><p className="mt-6 text-sm font-bold uppercase tracking-[0.18em] text-emerald-800">Bynex Digitalpärm</p><h3 className="mt-3 text-3xl font-semibold">19 kr/mån eller 190 kr/år</h3><p className="mt-2 text-sm font-semibold text-emerald-900">inklusive moms · per fastighet</p></div>
            <div><p className="text-lg leading-8 text-emerald-950">Kundportalen ingår i Bynex Bygg under hela projektet och i 12 månader efter avslut. Därefter kan fastighetsägaren behålla ritningar, garantier, protokoll och den publicerade projekthistoriken som en trygg digital pärm.</p><p className="mt-4 text-sm leading-6 text-emerald-900">Fortsättningen är frivillig och aktiveras av kunden. Betalning startar först när abonnemangsflöde, villkor och export är verifierade för produktion.</p></div>
          </div>
