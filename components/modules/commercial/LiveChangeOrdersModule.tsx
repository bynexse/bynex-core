...
export default function LiveChangeOrdersModule({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    {selected && <ChangeDrawer title={selected.change_order_number} change={selected} project={projectMap.get(selected.project_id)} projects={projects} saving={saving} canEdit={canManage && selected.status === "draft"} notify={notify} onPrepared={() => void load()} onClose={() => setSelected(null)} onSubmit={(event) => void save(event, selected)} />}
  </div>;
}

function ChangeDrawer({ title, change, project, projects, saving, canEdit = true, notify, onPrepared, onClose, onSubmit }: { title: string; change?: ChangeOrder; project?: Project; projects: Project[]; saving: boolean; canEdit?: boolean; notify?: (message: string) => void; onPrepared?: () => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="fixed inset-0 z-[70] flex justify-end bg-black/35"><div className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-emerald-700">{change ? statusLabel[change.status] ?? change.status : "ÄTA-utkast"}</p><h2 className="mt-1 text-3xl font-semibold">{title}</h2>{project && <p className="mt-2 text-sm text-zinc-500">{project.project_number} · {project.name}</p>}</div><button onClick={onClose} className="rounded-xl p-2 hover:bg-zinc-100" aria-label="Stäng"><X className="h-5 w-5" /></button></div>
    ...
    {change && canEdit && notify && <ChangeOrderPricePanel change={change} notify={notify} onPrepared={onPrepared} />}
...
function ChangeOrderPricePanel({ change, notify, onPrepared }: { change: ChangeOrder; notify: (message: string) => void; onPrepared?: () => void }) {
  ...
  async function prepare(event: FormEvent<HTMLFormElement>) {
    ...
    const values = Object.fromEntries(new FormData(event.currentTarget));
    const response = await fetch("/api/private/change-orders/approval-link",{ method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({ action:"prepare_and_link",changeOrderId:change.id,...values }) });
    const payload=await response.json().catch(() => null); setBusy(false);
    if (!response.ok) { setError(payload?.error ?? "Prisunderlaget kunde inte skickas."); return; }
    setLink(payload.approvalUrl); notify("Prisunderlaget är låst och kundlänken skapad"); onPrepared?.();
  }
  async function copy() { if (!link) return; await navigator.clipboard.writeText(link); notify("Kundlänken är kopierad"); }
  return <div className="mt-7 rounded-3xl border border-zinc-300 bg-zinc-50 p-5"><h3 className="text-xl font-semibold">Prisbesked till kund</h3><p className="mt-2 text-sm leading-6 text-zinc-600">Du granskar priset som behörig person. Därefter låses versionen och kunden får ett spårbart beslut – detta märks inte som BankID.</p>{error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-800">{error}</p>}{link ? <div className="mt-5"><input readOnly value={link} className="input" /><button type="button" onClick={() => void copy()} className="mt-3 w-full rounded-xl bg-zinc-950 px-4 py-3 text-sm font-semibold text-white">Kopiera kundlänk</button></div> : <form onSubmit={prepare} className="mt-5 space-y-4"><label className="block text-sm font-semibold">Kundbeskrivning<textarea name="customerDescription" defaultValue={change.description ?? ""} required minLength={2} maxLength={4000} rows={4} className="input mt-2" /></label><div className="grid grid-cols-2 gap-3"><PriceInput name="laborSell" label="Arbete exkl. moms" /><PriceInput name="materialSell" label="Material exkl. moms" /><PriceInput name="equipmentSell" label="Maskiner exkl. moms" /><PriceInput name="subcontractorSell" label="UE exkl. moms" /><PriceInput name="otherSell" label="Övrigt exkl. moms" /><PriceInput name="laborHours" label="Arbetstimmar" /></div><div className="grid grid-cols-2 gap-3"><label className="text-sm font-semibold">Prisform<select name="priceType" defaultValue="estimated" className="input mt-2"><option value="estimated">Uppskattat pris</option><option value="fixed">Fast pris</option><option value="running_account">Löpande räkning</option></select></label><label className="text-sm font-semibold">Moms %<input name="vatPercent" type="number" min="0" max="100" step="0.001" defaultValue="25" className="input mt-2" /></label></div><label className="block text-sm font-semibold">Avvikelsetext för uppskattat/löpande<textarea name="priceDisclaimer" rows={3} defaultValue="Priset är uppskattat och kan avvika vid ändrade förutsättningar. Om omfattningen ändras begär vi ett nytt godkännande innan merarbete påbörjas." className="input mt-2" /></label><label className="block text-sm font-semibold">Länken gäller dagar<input name="validDays" type="number" min="1" max="30" defaultValue="14" className="input mt-2" /></label><button disabled={busy} className="w-full rounded-xl bg-zinc-950 px-4 py-4 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Låser underlaget…" : "Granska, lås och skapa kundlänk"}</button></form>}</div>;
}

function PriceInput({ name,label }: { name:string;label:string }) { return <label className="text-sm font-semibold">{label}<input name={name} type="number" min="0" step="0.01" defaultValue="0" className="input mt-2" /></label>; }
