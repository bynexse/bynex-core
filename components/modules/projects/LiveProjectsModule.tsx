...
export default function LiveProjectsModule({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => setSelectedProject(requestedProject));
    return () => window.cancelAnimationFrame(frame);
  ...
  const filtered = useMemo(() => {
    ...
    return value
      ? projects.filter((project) =>
          [project.project_number, project.name, project.customer_name, project.city].some(
    ...
    <div className="space-y-5">
      <Card className="flex flex-col justify-between gap-6 bg-zinc-950 p-7 text-white sm:flex-row sm:items-end">
        <div>
          ...
          <h2 className="mt-5 text-4xl font-semibold tracking-tight">Projektet som nav</h2>
          <p className="mt-3 max-w-2xl text-zinc-300">
            Godkänd offert blir projekt med ett tryck. Projektets kunduppgifter och
            projektnummer följer sedan automatiskt till varje ÄTA.
          </p>
        </div>
        {canManage && (
          <button
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat icon={FolderKanban} label="Aktiva projekt" value={String(active)} helper="Planerade och pågående" />
        <Stat icon={CalendarDays} label="Alla projekt" value={String(projects.length)} helper="Inklusive avslutade" />
        <Stat icon={CircleAlert} label="Aktiv budget" value={currency.format(totalBudget)} helper="Registrerad projektbudget" />
      </div>
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {project.source_quote_id && <Badge tone="neutral">Från offert</Badge>}
                  </div>
                ...
                <div className="min-w-36">
                  ...
                  <div className="mt-2 h-2 overflow-hidden rounded-full bg-zinc-100">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, Math.max(0, Number(project.progress)))}%`,
                      }}
                    />
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold">{currency.format(Number(project.budget))}</p>
                  <button
                    type="button"
                    onClick={() => {
        ...
        <div className="fixed inset-0 z-[70] flex justify-end bg-black/35">
          <div className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-emerald-700">Nytt projekt</p>
                <h2 className="mt-1 text-3xl font-semibold">Minsta möjliga start</h2>
              </div>
              <button
                type="button"
            ...
            <p className="mt-3 text-sm leading-6 text-zinc-500">
              Fyll i det ni vet nu. Bynex skapar nästa projektnummer automatiskt och
              resten kan kompletteras senare.
            </p>
            <form onSubmit={createProject} className="mt-8 space-y-5">
              <label className="block">
                <span className="text-sm font-semibold">Projektnamn *</span>
                <input name="name" required minLength={2} maxLength={240} className="input mt-2" />
              </label>
              <label className="block">
                <span className="text-sm font-semibold">Kund *</span>
                <input name="customerName" required minLength={2} maxLength={200} className="input mt-2" />
              </label>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label>
                  <span className="text-sm font-semibold">Budget exkl. moms</span>
                  <input name="budget" type="number" min="0" step="1" defaultValue="0" className="input mt-2" />
                </label>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <label>
          ...
          <div className="h-full w-full max-w-3xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-3xl font-semibold">{selectedProject.name}</h2>
                <p className="mt-2 text-sm text-zinc-500">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs font-semibold">
              <div className={`rounded-xl px-3 py-3 ${selectedProject.source_quote_id ? "bg-emerald-100 text-emerald-900" : "bg-zinc-100 text-zinc-500"}`}>
                1. Offert
              </div>
            ...
            {selectedProject.source_quote_id && (
              <a
                href={`/app?module=quotes&quote=${encodeURIComponent(selectedProject.source_quote_id)}`}
            ...
            <div className="mt-5 grid gap-3 rounded-2xl bg-zinc-50 p-5 text-sm sm:grid-cols-2">
              <div>
                ...
                <p className="mt-1 font-semibold">
                  {[selectedProject.address, selectedProject.city].filter(Boolean).join(", ") || "Inte angiven"}
                </p>
              </div>
              <div>
            ...
            {canManage && selectedProject.active && !["completed", "cancelled"].includes(selectedProject.status) && (
              <div className="mt-6 rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-emerald-950/75">
                        Kund, kontakt och projektnummer återanvänds. Du behöver bara
                        beskriva vad som ändrats.
                      </p>
                    </div>
                  </div>
                  {!quickAtaOpen && (
                ...
                {quickAtaOpen && (
                  <form onSubmit={createChangeOrder} className="mt-5 space-y-4 rounded-2xl bg-white p-5">
                    <label className="block">
                    ...
                    <label className="block">
                      <span className="text-sm font-semibold">Kort beskrivning *</span>
                      <textarea
                        name="description"
                        required
                        minLength={2}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <label>
                        ...
                        <input name="requestedBy" maxLength={200} className="input mt-2" />
                      </label>
                      <label>
            ...
            {canManage ? (
              <form onSubmit={updateProject} className="mt-8 space-y-5">
                <label className="block">
                  ...
                  <select name="status" defaultValue={selectedProject.status} className="input mt-2">
                ...
                <label className="block">
                  <span className="text-sm font-semibold">Framdrift i procent</span>
                  <input
                    name="progress"
                    type="number"
                    min="0"
