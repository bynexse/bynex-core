...
export default function LiveAssetsRegistry({ notify }: { notify: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("bynex-assets-updated", refresh);
    return () => window.removeEventListener("bynex-assets-updated", refresh);
  }, [load]);
  ...
  const selected = data.assets.find((asset) => asset.id === selectedId && asset.status !== "archived") ?? null;
  const normalized = query.trim().toLowerCase();
  const activeAssets = data.assets.filter((asset) => asset.status !== "archived");
  const visibleAssets = activeAssets.filter((asset) => !normalized || [asset.asset_number, asset.name, asset.registration_number, asset.serial_number, asset.manufacturer, asset.model, asset.location_text].some((value) => value?.toLowerCase().includes(normalized)));
